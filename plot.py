...
N_tests = distance_from_optimal_tvar.shape[0]
T = distance_from_optimal_tvar.shape[2]
Steps_between_iterations = 10
N_iterations = residual_store.shape[1]
...
ax.plot(x, np.mean(residual_store, axis=0))
ax.grid(True)
ax.set_yscale('log')
# ax.set_xscale('log')
...
for index_K in range(len(N_iter_per_timestep)):
    ax.plot(x, np.mean(distance_from_optimal_tvar[:,index_K,:], axis=0), label=str(N_iter_per_timestep[index_K]),linestyle=next(linecycler))
ax.grid(True)
...
linecycler = cycle(lines)
for index_K in range(len(N_iter_per_timestep)):
    ax.plot(x, np.mean(shared_const_viol_tvar[:,index_K,:]+loc_const_viol_tvar[:,index_K,:], axis=0),\
